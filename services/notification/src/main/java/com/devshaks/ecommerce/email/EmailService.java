...
import com.devshaks.ecommerce.kafka.order.Product;
...
public class EmailService {
    ...
    public void sendPaymentSuccessEmail(String destinationEmail, String customerName, BigDecimal amount, String orderReference) throws MessagingException {
        ...
        messageHelper.setFrom("contact@devshaks.com");

        final String tempalteName = EmailTemplates.PAYMENT_CONFIRMATION.getTemplate();
        ...
        try {
            String htmlTemplate = templateEngine.process(tempalteName, context);
            messageHelper.setText(htmlTemplate, true);
            messageHelper.setTo(destinationEmail);
            mailSender.send(mimeMessage);
            log.info("Payment Confirmation Email Sent to: %s with %s " + destinationEmail, tempalteName);
        } catch (MessagingException e) {
            log.warn("Failed to Send Email to: %s with %s " + destinationEmail, tempalteName);
        }
    }
    ...
    public void sendOrderConfirmationEmail(String destinationEmail, String customerName, BigDecimal amount, String orderReference, List<Product> products) throws MessagingException {
        ...
        final String tempalteName = EmailTemplates.ORDER_CONFIRMATION.getTemplate();
        Map<String, Object> variables = new HashMap<>();
        variables.put("customerName", customerName);
        ...
        try {
            ...
            messageHelper.setTo(destinationEmail);
            mailSender.send(mimeMessage);
            log.info("Order Confirmation Email Sent to: %s with %s {}", destinationEmail, tempalteName);
        } catch (MessagingException e) {
            log.warn("Order to Send Email to: %s with %s {}", destinationEmail, tempalteName);
        }
    }
